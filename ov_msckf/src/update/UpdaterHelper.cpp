  ...
  for (auto const &pair : feature.timestamps) {
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      if (parity_jac_count < 2) {
        PRINT_DEBUG("[PARITY_JAC] obs_idx=%d timestamp=%.17g\n", parity_jac_count, feature.timestamps[pair.first].at(m));
        PRINT_DEBUG("[PARITY_JAC] p_FinC=%.17g,%.17g,%.17g\n", p_FinCi(0), p_FinCi(1), p_FinCi(2));
        PRINT_DEBUG("[PARITY_JAC] res=%.17g,%.17g\n", res(2*c), res(2*c+1));
        PRINT_DEBUG("[PARITY_JAC] H_f=%.17g,%.17g,%.17g;%.17g,%.17g,%.17g\n",
            H_f(2*c, 0), H_f(2*c, 1), H_f(2*c, 2),
            H_f(2*c+1, 0), H_f(2*c+1, 1), H_f(2*c+1, 2));
        // H_x clone block (first 6 columns for this observation's clone)
        PRINT_DEBUG("[PARITY_JAC] H_x_clone=%.17g,%.17g,%.17g,%.17g,%.17g,%.17g\n",
...
void UpdaterHelper::nullspace_project_inplace(Eigen::MatrixXd &H_f, Eigen::MatrixXd &H_x, Eigen::VectorXd &res) {
  ...
  PRINT_DEBUG("[PARITY_NULL] res_norm_after=%.17g\n", res.norm());
  if (H_x.rows() > 0) {
    ...
    for (int i = 0; i < std::min(10, (int)H_x.cols()); i++) {
      char buf[48];
      snprintf(buf, sizeof(buf), "%.17g%s", H_x(0, i), i < 9 ? "," : "...\n");
      hx_row0 += buf;
    }
